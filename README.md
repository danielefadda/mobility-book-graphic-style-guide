...
mbs.export_ase('mobility_colors.ase')

# Ottieni dizionario colori
colors = mbs.export_colors_dict()
print(f"Colori alias: {len(colors['aliases'])}")
print(f"Colori semantici: {len(colors['tokens'])}")
print(f"Category10: {len(colors['category10'])}")
```
...
- **[UPDATING.md](UPDATING.md)** - Come modificare i design token e gli alias colori
...
│   └── design_tokens.json      # Token semantici
├── _colors.py                  # Carica alias.json
├── _tokens.py                  # Carica design_tokens.json
├── matplotlib.py               # Builder per Matplotlib
├── altair.py                   # Builder per Altair
...
**I file JSON sono la source of truth**. Tutti i moduli Python leggono i token dai JSON al momento dell'import.

```
alias.json → ALIAS_COLORS → design_tokens.json → TOKENS → matplotlib/altair
```
...
### Token Semantici (design_tokens.json)

```json
{
  "semantic": {
    "brand-primary": { "value": "{colors.teal.500}" },
    "text-primary": { "value": "{colors.black.500}" },
    "bg-default": { "value": "{colors.white.500}" }
  }
}
```

I riferimenti `{alias.xxx}` e `{colors.xxx}` vengono risolti automaticamente.
...
1. Modifica `mobility_book_style/data/alias.json` o `design_tokens.json`
2. Valida il JSON: `python -m json.tool file.json > /dev/null`
...
- ✅ 110+ colori alias (teal, burgundy, purple, olive, gold, etc.)
- ✅ Token semantici (brand, text, background, etc.)
